<template>
  <main class="flex flex-col justify-start items-center">
    ...
    <div class="flex flex-row justify-between w-full fixed z-10 m-10">
      <button @click="toggleInvestStatus = !toggleInvestStatus"
      ...
      <button @click="toggleTechStatus = !toggleTechStatus"
    ...
    <div
      ...
      <div class="flex">
        <template v-for="(item, index) in investorType" :key="index">
          <input type="checkbox" :id="`${item.type}${index}`" class="peer hidden" v-model="investorSelectedType"
            :value="item.name" />
          <label :for="`${item.type}${index}`" class="select-none cursor-pointer rounded-lg border-2 border-gray-200
        py-3 px-6 font-bold text-gray-200 transition-colors duration-200 ease-in-out"
            :class="{ 'peer-checked:bg-gray-200 peer-checked:text-gray-900 peer-checked:border-gray-200': investorSelectedType.includes(item.name) }">
            {{ item.name }} </label>
        </template>
      </div>
      <p class="border-l-2 border-b-2 rounded-b-md border-red-500 p-2 mb-1">
      ...
      <div class="flex">
        <template v-for="(item, index) in marketType" :key="index">
          <input type="checkbox" :id="`${item.type}${index}`" class="peer hidden" v-model="marketSelectedType"
            :value="item.type" />
          <label :for="`${item.type}${index}`" class="select-none cursor-pointer rounded-lg border-2 border-gray-200
        py-3 px-6 font-bold text-gray-200 transition-colors duration-200 ease-in-out"
            :class="{ 'peer-checked:bg-gray-200 peer-checked:text-gray-900 peer-checked:border-gray-200': marketSelectedType.includes(item.type) }">
            {{ item.name }} </label>
        </template>
      </div>
      <p class="border-l-2 border-b-2 rounded-b-md border-red-500 p-2 mb-1">
      ...
      <div class="flex">
        <template v-for="(item, index) in accumDayType" :key="index">
          <input type="checkbox" :id="`${item.type}${index}`" class="peer hidden" v-model="accumDaySelectedType"
            :value="item.type" />
          <label :for="`${item.type}${index}`" class="select-none cursor-pointer rounded-lg border-2 border-gray-200
        py-3 px-6 font-bold text-gray-200 transition-colors duration-200 ease-in-out"
            :class="{ 'peer-checked:bg-gray-200 peer-checked:text-gray-900 peer-checked:border-gray-200': accumDaySelectedType.includes(item.type) }">
            {{ item.name }} </label>
        </template>
      </div>
      <button v-if="false" @click="getInvestCombineData(content)" class="rounded-md border border-gray-500 p-2 mb-1">
        Search
      </button>
    </div>
    <!-- 主要內容table -->
    <div class="flex flex-row justify-center items-start w-full sm:flex-col sm:items-center mt-8 sm:mt-40">
      <StockInvestDBTableVue :investData="buy_page_list" title="買超" @openModal="troggleModalF" class="sm:mt-2" />
      <StockInvestDBTableVue :investData="sell_page_list" title="賣超" @openModal="troggleModalF" class="sm:mt-2" />
    </div>
    <!-- 跳窗 -->
    <div class="fixed top-50 z-30" v-if="modalStatus">
      <StockModalLink @closeModal="troggleModalF" :propsToModal="propsToModal" :linkWeb="linkWeb" />
    </div>
    <!-- 方塊顯示點選到的法人長方形 -->
...
export default {
  ...
  setup() {
    ...
    const title = computed(() =>{
      const a = investorSelectedType.value.concat()
      console.log(a)
      console.log(investorSelectedType.value.concat())
    })
    onMounted(async () => {
